...
pub enum StaticError {
    ...
    #[error("An operator is specified for a field of a type to which the operator is not applicable.")]
    S2,
...
pub enum DynamicError {
    ...
    #[error("A field in a template cannot be converted to or from the type of the corresponding application field")]
    D1,
    ...
    /// field.
    #[error("An integer in the stream does not fall within the bounds of the specific integer type specified on the corresponding field.")]
    D2,
    ...
    /// mantissa.
    #[error("A decimal value cannot be encoded due to limitations introduced by using individual operators on exponent and mantissa.")]
    D3,
    /// It is a dynamic error if the type of the previous value is not the same
    /// as the type of the field of the current operator.
    #[error("The type of the previous value is not the same as the type of the field of the current operator.")]
    D4,
    ...
    /// instruction context.
    #[error("A mandatory field is not present in the stream, has an undefined previous value and there is no initial value in the instruction context.")]
    D5,
    /// It is a dynamic error if a mandatory field is not present in the stream
    ...
    #[error("The subtraction length exceeds the length of the base value or if it does not fall in the value rang of an int32.")]
    D7,
    ...
    #[error("The name specified on a static template reference does not point to a template known by the encoder or decoder.")]
    D8,
    /// It is a dynamic error if a decoder cannot find a template associated with
    /// a template identifier appearing in the stream.
    #[error("A decoder cannot find a template associated with a template identifier appearing in the stream.")]
    D9,
...
pub enum ReportableError {
    ...
    /// int64.
    #[error("A decimal cannot be represented by an exponent in the range [-63 … 63] or if the mantissa does not fit in an int64.")]
    R1,
    ...
    #[error("The combined value after applying a tail or delta operator to a Unicode string is not a valid UTF-8 sequence.")]
    R2,
    ...
    /// set.
    #[error("A Unicode string that is being converted to an ASCII string contains characters that are outside the ASCII character set.")]
    R3,
    /// It is a reportable error if a value of an integer type cannot be
    /// represented in the target integer type in a conversion.
    #[error("A value of an integer type cannot be represented in the target integer type in a conversion.")]
    R4,
    ...
    /// integer type.
    #[error("A decimal being converted to an integer has a negative exponent or if the resulting integer does not fit the target integer type.")]
    R5,
