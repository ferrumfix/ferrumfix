...
pub fn heartbeat_exact_with_context(secs: u64, raw_message: Option<&[u8]>) -> SmartString {
    let base_msg = format!("Invalid HeartBtInt(108), expected value {secs} seconds");
    if let Some(raw) = raw_message {
        format!("{base_msg} (raw message: {})", hex_debug(raw)).into()
    } else {
        base_msg.into()
    }
}

pub fn heartbeat_exact(secs: u64) -> SmartString {
...
pub fn heartbeat_range_with_context(a: u64, b: u64, raw_message: Option<&[u8]>) -> SmartString {
...
pub fn heartbeat_range(a: u64, b: u64) -> SmartString {
    heartbeat_range_with_context(a, b, None)
}

/// Enhanced error message for heartbeat > 0 validation with optional raw message context.
pub fn heartbeat_gt_0_with_context(raw_message: Option<&[u8]>) -> SmartString {
    let base_msg = "Invalid HeartBtInt(108), expected value greater than 0 seconds";
    if let Some(raw) = raw_message {
        format!("{base_msg} (raw message: {})", hex_debug(raw)).into()
    } else {
        base_msg.to_string().into()
    }
}

pub fn heartbeat_gt_0() -> SmartString {
    heartbeat_gt_0_with_context(None)
}

pub fn inbound_seqnum() -> SmartString {
    "NextExpectedMsgSeqNum(789) > than last message sent"
        .to_string()
        .into()
}

/// Enhanced error message for sequence number validation with optional raw message context.
pub fn msg_seq_num_with_context(seq_number: u64, raw_message: Option<&[u8]>) -> SmartString {
...
pub fn msg_seq_num(seq_number: u64) -> SmartString {
...
pub fn production_env_with_context(raw_message: Option<&[u8]>) -> SmartString {
    let base_msg =
...
pub fn production_env() -> SmartString {
    production_env_with_context(None)
}

/// Enhanced error message for missing fields with optional raw message context.
pub fn missing_field_with_context(name: &str, tag: u32, raw_message: Option<&[u8]>) -> SmartString {
...
pub fn missing_field(name: &str, tag: u32) -> SmartString {
...
fn hex_debug(raw: &[u8]) -> SmartString {
