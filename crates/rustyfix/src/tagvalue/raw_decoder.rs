    ...
    T: AsRef<[u8]>,
{
    ...
    ///
    /// ```
    /// use rustyfix::tagvalue::{Config, RawDecoder};
    ...
    B: Buffer,
{
    ...
    ///
    /// # Panics
    /// This function will panic if the message is not fully decoded. Check
    /// `try_parse` return value before calling this function.
    pub fn raw_frame(&self) -> RawFrame<&[u8]> {
        if let ParserState::Header(header_info, _len) = &self.state {
            ...
            panic!(
                "The message is not fully decoded or the decoder is in a failed state. Check `try_parse` return value."
            );
        }
    }
}
